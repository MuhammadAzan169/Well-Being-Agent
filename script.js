...
<<<<<<< HEAD
    ...
    let sourcesHtml = '';
    if (sender === 'system' && sources && sources.length > 0) {
        const sourceItems = sources.slice(0, 3).map(s => {
            const topic = s.topic || 'General';
            const source = s.source || '';
            const score = s.score ? `${Math.round(s.score * 100)}%` : '';
            const sourceText = source ? `${topic} — ${source}` : topic;
            return `<span class="source-badge" title="Relevance: ${score}"><i class="fas fa-bookmark"></i> ${sourceText}</span>`;
        }).join('');
